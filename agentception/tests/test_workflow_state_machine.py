...
2. Open PR with no body but branch ``agent/issue-17`` → lane ``pr_open``
...
def _best_pr(
    ...
    pr_head_ref: str | None = "agent/issue-17",
...
def _pr_row(
    ...
    head_ref: str | None = "agent/issue-17",
...
class TestLaneComputation:
    ...
    def test_open_pr_with_branch_regex_is_pr_open(self) -> None:
        """Acceptance criterion 2: no body but agent/issue-17 branch."""
        result = compute_workflow_state(
            _issue(),
            _run(),
...
class TestLinkDiscovery:
    ...
    def test_branch_regex(self) -> None:
        """agent/issue-{N} branch convention is detected as branch_regex at confidence 90."""
        pr = _pr_row(head_ref="agent/issue-17")
        links = discover_links_for_pr(pr, _REPO)
    ...
    def test_old_feat_branch_not_matched(self) -> None:
        """Legacy feat/issue-{N} and ac/issue-{N} branches are no longer matched."""
        for head_ref in ("feat/issue-17-fix-buttons", "ac/issue-17"):
            pr = _pr_row(head_ref=head_ref)
            links = discover_links_for_pr(pr, _REPO)
            branch_links = [l for l in links if l["link_method"] == "branch_regex"]
            assert branch_links == [], f"Expected no branch_regex link for {head_ref!r}"
...
class TestBestPRSelection:
    ...
    def test_prefers_open_over_merged(self) -> None:
        ...
        pr_info = {
            1: PRInfo(number=1, state="merged", base_ref="dev", head_ref="agent/issue-17"),
            2: PRInfo(number=2, state="open", base_ref="dev", head_ref="agent/issue-17-b"),
        }
...
class TestEndToEndLaneDerivation:
    ...
    def test_branch_convention_drives_pr_open(self) -> None:
        """Acceptance criterion 2: agent/issue-17 branch → pr_open."""
        pr = _pr_row(number=42, head_ref="agent/issue-17", body="")
        links = discover_links_for_pr(pr, _REPO)
        pr_info = {42: PRInfo(number=42, state="open", base_ref="dev", head_ref="agent/issue-17")}
        best = best_pr_for_issue(17, links, pr_info)
        result = compute_workflow_state(_issue(number=17), _run(), best)
        assert result["lane"] == LANE_PR_OPEN
    ...
    def test_wrong_base_still_pr_open(self) -> None:
        ...
        pr = _pr_row(number=42, head_ref="agent/issue-17", body="Closes #17", base_ref="main")
        links = discover_links_for_pr(pr, _REPO)
        pr_info = {42: PRInfo(number=42, state="open", base_ref="main", head_ref="agent/issue-17")}
        best = best_pr_for_issue(17, links, pr_info)
        result = compute_workflow_state(_issue(number=17), _run(), best)
        assert result["lane"] == LANE_PR_OPEN
        assert any("wrong_base" in w for w in result["warnings"])
