...
However, **three writer paths were not migrated** (`dispatch.py` × 2 and `mcp/plan_tools.py` × 1), **all template reader sections** (7 files: `agent-engineer.md`, `agent-reviewer.md`, `agent-conductor.md`, and the four role files `cto.md`, `engineering-coordinator.md`, `qa-coordinator.md`, `pr-reviewer.md`) still use `grep "^KEY="` shell commands that will fail against TOML v2 files, and **nine test files** contain fixtures or assertions anchored to the old flat K=V format. Note: `agent-triage.md` was deleted (pre-pipeline legacy — superseded by Phase 1A/1B + issue_creator).
...
| 13 | `.agentception/agent-triage.md` | 423, 458, 464–468 | `template` | **deleted** — Pre-pipeline legacy. The "bugs-to-issues" coordinator workflow has been fully superseded by the Phase 1A (LLM → PlanSpec YAML) + Phase 1B (human review) + `issue_creator.py` pipeline. Template and generated file removed. | deleted |
...
| `.agentception/agent-triage.md` | — | ✅ deleted (pre-pipeline legacy) |
