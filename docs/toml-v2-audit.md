...
However, **three writer paths were not migrated** (`dispatch.py` × 2 and `mcp/plan_tools.py` × 1), **all template reader sections** (8 files: `agent-engineer.md`, `agent-reviewer.md`, `agent-triage.md`, `agent-conductor.md`, and the four role files `cto.md`, `engineering-coordinator.md`, `qa-coordinator.md`, `pr-reviewer.md`) still use `grep "^KEY="` shell commands that will fail against TOML v2 files, and **nine test files** contain fixtures or assertions anchored to the old flat K=V format.
...
| `.agentception/agent-triage.md` | GH_REPO, ROLE, PHASE_LABEL, LABELS_TO_APPLY, PHASE_DEPENDS_ON_ISSUES, ATTEMPT_N | ❌ Flat K=V grep |
