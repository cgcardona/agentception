...
| 11 | `.agentception/parallel-issue-to-pr.md` | 500–504, 524, 533, 672, 722 | `template` | Agent reader instructions use `grep "^KEY=" .agent-task \| cut -d= -f2` to extract values. Writer section was correctly migrated to TOML v2 in `aaf8a75`, but reader section was not updated. Broken fields: `GH_REPO` (`[repo].gh_repo`), `ISSUE_NUMBER` (`[target].issue_number`), `ATTEMPT_N` (`[task].attempt_n`), `BATCH_ID` (`[pipeline].batch_id`), `ROLE` (`[agent].role`), `COGNITIVE_ARCH` (`[agent].cognitive_arch`), `DEPENDS_ON` (`[target].depends_on`), `FILE_OWNERSHIP` (`[target].file_ownership`). | `aaf8a75` (writer migrated; reader left behind) |
