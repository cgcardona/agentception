...
The public entry points branch on `settings.effective_llm_provider` and call the corresponding adapter. The local adapter talks to **your** Ollama server using the same **request/response shape** as OpenAI’s Chat Completions API—“OpenAI” here means **wire format**, not OpenAI Inc.; no cloud call is required. **Generation budget:** callers may pass large `max_tokens` (e.g. 8192 for Plan 1A); the local adapter clamps to `settings.local_llm_completion_token_ceiling` (default **8192** for Ollama; lower via `LOCAL_LLM_COMPLETION_TOKEN_CEILING` if your server has a stricter cap). Responses are normalized (e.g. `content` as string or list of parts; reasoning stripped) so the contract is always satisfied.
