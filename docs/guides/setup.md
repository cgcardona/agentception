...
| `LLM_PROVIDER` | Optional | Which LLM backend to use: `anthropic` (default) or `local`. When `local`, all LLM calls go to an OpenAI-compatible server (see [Local LLM with MLX](local-llm-mlx.md)). | Default: `anthropic` |
